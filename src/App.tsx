...
const PRODUCTS = [
  { id:1, kanji:"企", color:"#e67e22", tag:"制作", name:"企画制作",     price:300000, costPrice:120000, supplierId:1 },
  { id:2, kanji:"撮", color:"#c0392b", tag:"制作", name:"撮影",         price:150000, costPrice:80000,  supplierId:2 },
  { id:3, kanji:"編", color:"#8e44ad", tag:"制作", name:"映像編集",     price:200000, costPrice:90000,  supplierId:2 },
  { id:4, kanji:"印", color:"#16a085", tag:"印刷", name:"印刷物制作",   price:80000,  costPrice:55000,  supplierId:3 },
  { id:5, kanji:"研", color:"#d4ac0d", tag:"研修", name:"研修・セミナー",price:250000, costPrice:60000,  supplierId:4 },
  { id:6, kanji:"運", color:"#27ae60", tag:"運用", name:"運用管理",     price:120000, costPrice:30000,  supplierId:5 },
  { id:7, kanji:"稿", color:"#ca6f1e", tag:"制作", name:"原稿・コピー", price:90000,  costPrice:40000,  supplierId:6 },
  { id:8, kanji:"設", color:"#2471a3", tag:"設計", name:"システム設計", price:350000, costPrice:180000, supplierId:1 },
  { id:9, kanji:"策", color:"#a93226", tag:"企画", name:"戦略策定",     price:400000, costPrice:100000, supplierId:1 },
];

const TAGS = ["すべて", ...new Set(PRODUCTS.map(p => p.tag))];

const Badge = ({ kanji, color, size=40 }) => (
  <div style={{
...
export default function App() {
  ...
  const chgQty = (uid, d) => setRows(r => r.map(row => row.uid===uid ? {...row, qty:Math.max(1,row.qty+d)} : row));
  const del = (uid) => setRows(r => r.filter(row => row.uid!==uid));
